...
import { SystemStatus, ModelInfo } from "@/lib/types";
  ...
  return (
    <header className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-border bg-panel/70 px-5 backdrop-blur">
      ...
      <div className="flex items-center gap-2.5">
        <ModelPicker models={models} value={selectedModel} disabled={streaming} onChange={onSelectModel} />
        <StatusComp status={status} />
        <button
          onClick={onToggleInspector}
          title={inspectorOpen ? "Hide pipeline inspector" : "Show pipeline inspector"}
          aria-pressed={inspectorOpen}
          className={`hidden items-center gap-1.5 rounded-md border px-2 py-1 text-xs font-medium transition-colors lg:flex ${
            inspectorOpen
