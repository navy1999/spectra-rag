...
Each substitutes for — or, where the model supports it, *drives* — an LLM sampling control. The honest status of each is the interesting part:
...
| 2 | **Trie Stream Interceptor** — rewrites near-miss entity names (Levenshtein) to canonical graph spellings. | `logit_bias` (rarely supported) | **Justified workaround**, but a *conditional* safety net — fires only when the model misspells; near-zero on clean graph-RAG. | [`trie/interceptor.go`](backend/trie/interceptor.go) |
...
The project's value is not asserted, it's measured — with judge-free string metrics, leave-one-out cross-validation with permutation tests, and a blind LLM-as-judge harness. Including the results that came back negative.
...
The permutation test is the key rigor check: shuffling the labels collapses accuracy to chance, so the 97.5% is genuine separation, not overfitting. **Honest caveat:** it's 40 author-written queries, so this rules out *noise*, not *authorship bias* — an independent held-out set is the next step. The path that got here (an early *negative* result — unsupervised PCA losing to a length heuristic because the dataset was secretly length-separable — and the dataset rebuild + supervised fix that followed) is documented in the git history and is worth more than a cherry-picked number.
...
The deployed backend loads `data/lda_router.json` (overrides the path decision) and `data/node_embeddings.json` (semantic seeding); both degrade gracefully if absent. Projection is a pure-Go `components·(x−mean)` matvec — no cgo required (`CGO_ENABLED=0`, the Railway default). The C++/Eigen engine (`go build -tags cgo_pca`) is an optional fast path.
